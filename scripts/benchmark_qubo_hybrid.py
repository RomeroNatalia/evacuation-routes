...
4. Repeats D-Wave Leap Hybrid across independent jobs (10 by default).
5. Selects the best valid run and writes its detailed result files.
6. Writes aggregate benchmark run and summary CSV files.
...
Run from the repository root:
    ...
    python scripts/benchmark_qubo_hybrid.py

Optionally choose a different number of runs:

    python scripts/benchmark_qubo_hybrid.py 10
...
Generated outputs:
    ...
    data/floorplans/FPXX/output/qubo_hybrid/benchmark_runs.csv
    data/floorplans/FPXX/output/qubo_hybrid/benchmark_summary.csv
    data/floorplans/FPXX/output/qubo_hybrid/hybrid_samples.csv
    data/floorplans/FPXX/output/qubo_hybrid/selected_assignments.csv
    data/floorplans/FPXX/output/qubo_hybrid/edge_congestion_report.csv
    data/floorplans/FPXX/output/qubo_hybrid/exit_usage_report.csv
    data/floorplans/FPXX/output/qubo_hybrid/solution_summary.json
    data/floorplans/FPXX/output/qubo_hybrid/solution_summary.txt
"""
...
OUTPUT_DIR = INPUT_DIR / "qubo_hybrid"
...
def solve_with_hybrid(bqm, run_number: int = 1, sampler=None):
    """Submit one independent normalized BQM trial to LeapHybridSampler."""
    if sampler is None:
        sampler = LeapHybridSampler()
    label = f"{HYBRID_LABEL} - Run {run_number}"
...
def main() -> None:
    ...
    print(f"\nRunning {run_count} independent Leap hybrid trials...")
    sampler = LeapHybridSampler()
    benchmark_rows = []
    run_results = []
    ...
    print("\nOFFICE EVACUATION QUBO HYBRID BENCHMARK COMPLETE")
    print("=" * 52)
    print("Solver:                 D-Wave LeapHybridSampler")

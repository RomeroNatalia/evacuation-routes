...
Run from the repository root:
    ...
    python scripts/solve_qubo_qpu.py
...
def solve_with_qpu(bqm):
    """Submit the BQM with explicit gauges and chain-strength control."""
    qpu = DWaveSampler()
...
def main() -> None:
    ...
    sampleset, qpu, reads_per_transform = solve_with_qpu(bqm)
    ...
    print("\nOFFICE EVACUATION QUBO DIRECT QPU RUN COMPLETE")
    print("=" * 48)
    ...
    print(f"QPU problem ID:         {sampleset.info.get('problem_id')}")
    print(
        "Best chain break:       "
        f"{float(getattr(best, 'chain_break_fraction', 0.0)):.6f}"
    )

...
4. Embeds the BQM and submits it directly to a D-Wave quantum processing unit (QPU).
...
Run from the repository root:
    ...
    python scripts/benchmark_qubo_qpu.py

Optionally choose a different number of runs:

    python scripts/benchmark_qubo_qpu.py 10
...
Generated outputs:
    ...
    data/floorplans/FPXX/output/qubo_qpu/benchmark_runs.csv
    data/floorplans/FPXX/output/qubo_qpu/benchmark_summary.csv
    data/floorplans/FPXX/output/qubo_qpu/qpu_samples.csv
    data/floorplans/FPXX/output/qubo_qpu/selected_assignments.csv
    data/floorplans/FPXX/output/qubo_qpu/edge_congestion_report.csv
    data/floorplans/FPXX/output/qubo_qpu/exit_usage_report.csv
    data/floorplans/FPXX/output/qubo_qpu/solution_summary.json
    data/floorplans/FPXX/output/qubo_qpu/solution_summary.txt
"""
...
# Independent benchmark repetitions. Override with a command-line argument.
DEFAULT_NUM_RUNS = 10
...
def solve_with_qpu(bqm, run_number: int = 1):
    """Submit one independent QPU trial with gauges and chain-strength control."""
    qpu = DWaveSampler()
    ...
    sampleset = sampler.sample(
        ...
        label=f"{QPU_LABEL} - Run {run_number}",
    )

    return sampleset, qpu, reads_per_transform
...
def parse_benchmark_run_count() -> int:
    """Read an optional positive run count from the command line."""
    if len(sys.argv) == 1:
        return DEFAULT_NUM_RUNS
    if len(sys.argv) != 2:
        raise SystemExit(
            f"Usage: python {Path(sys.argv[0]).name} [number_of_runs]"
        )

    try:
        run_count = int(sys.argv[1])
    except ValueError as exc:
        raise SystemExit("number_of_runs must be a positive integer.") from exc

    if run_count <= 0:
        raise SystemExit("number_of_runs must be a positive integer.")
    return run_count

def main() -> None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    MODEL_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    run_count = parse_benchmark_run_count()
    ...
    print(f"\nRunning {run_count} independent direct-QPU trials...")
    benchmark_rows = []
    run_results = []

    for run_number in range(1, run_count + 1):
        print(f"  Run {run_number}/{run_count}...", end="", flush=True)
        started = time.perf_counter()
        sampleset, qpu, reads_per_transform = solve_with_qpu(
            bqm,
        ...
        result.update(
            {
                ...
                "sampleset": sampleset,
                "qpu": qpu,
                "reads_per_transform": reads_per_transform,
        ...
        benchmark_rows.append(
            {
                "solver": "dwave.system.DWaveSampler + EmbeddingComposite",
                "run": run_number,
                "problem_id": sampleset.info.get("problem_id", ""),
                "best_energy": float(result["best"].energy),
                "valid_best_solution": result["valid"],
                "valid_sample_rate": float(result["sample_stats"]["valid_sample_rate"]),
        ...
        print(
            ...
            f"chain_break={best_chain_break:.6f}, "
            f"time={solver_seconds:.3f}s"
        )

    valid_results = [result for result in run_results if result["valid"]]
    candidate_results = valid_results if valid_results else run_results
    best_result = min(
        candidate_results,
    ...
    summary = write_solution_outputs(
        ...
        best_result["qpu"],
        best_result["reads_per_transform"],
    )

    print("\nOFFICE EVACUATION QUBO DIRECT-QPU BENCHMARK COMPLETE")
    print("=" * 56)
    print("Solver:                 D-Wave QPU via EmbeddingComposite")
    print(f"Independent runs:       {run_count}")
    ...
    print(f"Congestion objective:   {best_result['congestion_objective']:.6f}")
    print(f"Overloaded edges:       {best_result['overloaded_edge_count']}")
    print(f"Run data:               {OUTPUT_DIR / 'benchmark_runs.csv'}")
    print(f"Benchmark summary:      {OUTPUT_DIR / 'benchmark_summary.csv'}")

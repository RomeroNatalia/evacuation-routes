...
4. Sweeps several exactly-one assignment penalties with Neal simulated annealing.
5. Repeats each penalty across independent multi-seed trials while holding other settings fixed.
6. Reports feasibility, energy, route distance, congestion, overload, and timing.
...
Run from the repository root:
    ...
    python scripts/sweep/sweep_assignment_penalty_neal.py
...
Generated outputs:
    ...
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/qubo_variable_index.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/qubo_linear_coefficients.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/qubo_quadratic_coefficients.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/qubo_upper_triangular.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/qubo_dense_xtqx.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/neal_samples.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/selected_assignments.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/edge_congestion_report.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/exit_usage_report.csv
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/solution_summary.json
    data/floorplans/FPXX/output/qubo_neal_penalty_sweep/solution_summary.txt
"""
...
OUTPUT_DIR = INPUT_DIR / "qubo_neal_penalty_sweep"
...
def solve_with_neal(bqm, run_number: int = 1):
    """Run one independent multi-seed simulated-annealing trial."""
    sampler = neal.SimulatedAnnealingSampler()
    base_seeds = tuple(int(seed) for seed in SEEDS)
    if not base_seeds:
        raise ValueError("SEEDS must contain at least one integer.")

    # Each benchmark run gets a different deterministic seed set.
    # This keeps the experiment reproducible without repeating identical trials.
    seed_offset = (max(1, int(run_number)) - 1) * 1000
    seeds = tuple(
        ((seed + seed_offset) % 2_147_483_646) + 1
    ...
    return sampleset, reads_per_seed, seeds
...
def write_penalty_sweep_outputs(
    ...
    """Write one row per run plus one aggregate row per tested penalty."""
    if not run_rows:
        raise ValueError("No simulated-annealing penalty-sweep runs were recorded.")

    write_csv(
        OUTPUT_DIR / "neal_penalty_sweep_runs.csv",
    ...
    write_csv(
        OUTPUT_DIR / "neal_penalty_sweep_summary.csv",
...
def main() -> None:
    ...
    print("\nSIMULATED-ANNEALING ASSIGNMENT-PENALTY SWEEP")
    print("=" * 58)
    ...
    print(f"Nominal reads per run: {NUM_READS}")
    print(f"Sweeps per read: {NUM_SWEEPS}")
    print(
        "Held fixed: normalization, objective weights, pruning, reads, "
        "sweeps, and the current multi-seed strategy."
    )

    all_run_rows = []
    all_results = []

    # Make each penalty/run combination use a distinct deterministic seed offset.
    global_run_number = 0
    ...
    for penalty_index, (assignment_penalty, penalty_source) in enumerate(
        ...
        for run_number in range(1, runs_per_penalty + 1):
            ...
            sampleset, reads_per_seed, seeds_used = solve_with_neal(
                bqm,
                run_number=global_run_number,
            )
    ...
    summary = write_solution_outputs(
        ...
        best_result["reads_per_seed"],
        best_result["seeds_used"],
    )

    print("\nSIMULATED-ANNEALING PENALTY SWEEP COMPLETE")
    print("=" * 58)
    print(
    ...
    print(
        f"Best run valid sample rate:  "
        f"{100.0 * best_result['sample_stats']['valid_sample_rate']:.2f}%"
    )
    ...
    print(
        "Per-run results:             "
        f"{OUTPUT_DIR / 'neal_penalty_sweep_runs.csv'}"
    )
    print(
        "Per-penalty summary:         "
        f"{OUTPUT_DIR / 'neal_penalty_sweep_summary.csv'}"
    )

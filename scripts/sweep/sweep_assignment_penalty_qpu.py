...
4. Sweeps several exactly-one assignment penalties on the direct D-Wave QPU.
5. Repeats each penalty across independent QPU jobs with checkpoint/resume support.
6. Reports feasibility, energy, route distance, congestion, overload, chain breaks, and timing.
...
Run from the repository root:
    ...
    python scripts/sweep/sweep_assignment_penalty_qpu.py

Optionally set the number of independent runs per penalty:

    python scripts/sweep/sweep_assignment_penalty_qpu.py 5
...
Generated outputs:
    ...
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qpu_penalty_sweep_runs.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qpu_penalty_sweep_summary.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qubo_variable_index.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qubo_linear_coefficients.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qubo_quadratic_coefficients.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qubo_upper_triangular.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qubo_dense_xtqx.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/qpu_samples.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/selected_assignments.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/edge_congestion_report.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/exit_usage_report.csv
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/solution_summary.json
    data/floorplans/FPXX/output/qubo_qpu_penalty_sweep/solution_summary.txt
"""
...
OUTPUT_DIR = INPUT_DIR / "qubo_qpu_penalty_sweep"
...
# Direct-QPU sampling settings.
QPU_LABEL = f"{FLOORPLAN.floorplan_id} Fire Evacuation QUBO - Direct QPU"
NUM_READS = 1000
NUM_SPIN_REVERSAL_TRANSFORMS = 4
CHAIN_STRENGTH_PREFACTOR = 1.5
ANNEALING_TIME = 20.0
ANSWER_MODE = "histogram"

# Controlled assignment-penalty experiment.
# None means "use the automatically calculated penalty" as the baseline.
# Every other QPU/QUBO setting is intentionally held fixed during the sweep.
...
def create_qpu_sampler():
    """Create the direct-QPU sampler stack once and reuse it for the sweep."""
    qpu = DWaveSampler()
    physical_sampler = SpinReversalTransformComposite(qpu)
...
def solve_with_qpu(
    ...
    """Submit one QPU trial using the already-created sampler stack."""
    transforms = max(1, int(NUM_SPIN_REVERSAL_TRANSFORMS))
    reads_per_transform = max(1, math.ceil(NUM_READS / transforms))
    chain_strength = partial(
        uniform_torque_compensation,
        prefactor=CHAIN_STRENGTH_PREFACTOR,
    )

    sampleset = sampler.sample(
        bqm,
        num_reads=reads_per_transform,
        num_spin_reversal_transforms=transforms,
        ...
        label=f"{QPU_LABEL} - {penalty_label} - Run {run_number}",
    )

    return sampleset, reads_per_transform
...
def export_samples(sampleset, room_variables) -> None:
    ...
    write_csv(OUTPUT_DIR / "qpu_samples.csv", fieldnames, sample_rows)
...
def write_solution_outputs(
    ...
    """Write decoded assignments, QPU diagnostics, and current metrics."""
    assignment_rows = [
        {
            "room_id": route["room_id"],
            "room_name": route["room_name"],
    ...
    embedding_context = dict(sampleset.info.get("embedding_context", {}))
    embedding = embedding_context.get("embedding", {})
    chain_lengths = [len(chain) for chain in embedding.values()]

    total_chain_occurrences = 0
    weighted_chain_break_sum = 0.0
    if "chain_break_fraction" in sampleset.record.dtype.names:
        for datum in sampleset.data(
            fields=["chain_break_fraction", "num_occurrences"]
        ):
            occurrences = int(getattr(datum, "num_occurrences", 1))
            total_chain_occurrences += occurrences
            weighted_chain_break_sum += (
                float(datum.chain_break_fraction) * occurrences
            )

    try:
        solver_identity = qpu.solver.identity.dict()
    except Exception:
        solver_identity = {
            "name": getattr(qpu.solver, "name", None),
            "id": getattr(qpu.solver, "id", None),
        }

    actual_chain_strength = embedding_context.get("chain_strength")
    if actual_chain_strength is not None:
        try:
            actual_chain_strength = float(actual_chain_strength)
        except (TypeError, ValueError):
            actual_chain_strength = str(actual_chain_strength)

    summary = {
        "solver": "dwave.system.DWaveSampler + EmbeddingComposite + SpinReversalTransformComposite",
        "formulation_version": "normalized_tight_penalty_v2",
        "qpu_label": QPU_LABEL,
        "problem_id": problem_id,
        "qpu_solver_identity": solver_identity,
        "timing": timing,
        "requested_total_reads": NUM_READS,
        "expected_total_reads": reads_per_transform * max(1, int(NUM_SPIN_REVERSAL_TRANSFORMS)),
        "reads_per_spin_reversal": reads_per_transform,
        "num_spin_reversal_transforms": NUM_SPIN_REVERSAL_TRANSFORMS,
        "annealing_time_microseconds": ANNEALING_TIME,
        "answer_mode": ANSWER_MODE,
        "chain_strength_prefactor": CHAIN_STRENGTH_PREFACTOR,
        "actual_chain_strength": actual_chain_strength,
        "embedding_logical_variable_count": len(embedding),
        "embedding_physical_qubit_count": sum(chain_lengths),
        "embedding_max_chain_length": max(chain_lengths, default=0),
        "embedding_average_chain_length": (
            sum(chain_lengths) / len(chain_lengths) if chain_lengths else 0.0
        ),
        "best_chain_break_fraction": float(
            getattr(best, "chain_break_fraction", 0.0)
        ),
        "average_chain_break_fraction": (
            weighted_chain_break_sum / total_chain_occurrences
            if total_chain_occurrences else 0.0
        ),
    ...
    solver_text = "Solver: direct D-Wave QPU with embedding and spin reversals"
...
def parse_bool(value) -> bool:
    """Convert CSV/string/bool values to a real boolean."""
    if isinstance(value, bool):
        return value
    return str(value).strip().lower() in {"1", "true", "yes", "y"}


def load_checkpoint_rows(path: Path) -> List[Dict[str, object]]:
    """Load already-completed sweep runs so an interrupted sweep can resume."""
    if not path.exists():
        return []
    ...
    float_fields = {
        ...
        "qpu_sampling_time_microseconds",
    }

    for row in rows:
        item = dict(row)
        item["valid_best_solution"] = parse_bool(item["valid_best_solution"])
...
def write_penalty_sweep_outputs(
    ...
    """Write checkpoint rows and one aggregate row per tested penalty."""
    if not run_rows:
        return

    runs_path = OUTPUT_DIR / "qpu_penalty_sweep_runs.csv"
    write_csv(
        runs_path,
    ...
    for penalty in sorted(rows_by_penalty):
        ...
        valid_rows = [
            row for row in rows if parse_bool(row["valid_best_solution"])
        ]
        energy_pool = valid_rows if valid_rows else rows
        best_row = min(
            energy_pool,
            key=lambda row: float(row["best_energy"]),
        )
        ...
        summary = {
            ...
            "best_overloaded_edge_count": int(
                float(best_row["overloaded_edge_count"])
            ),
    ...
    write_csv(
        OUTPUT_DIR / "qpu_penalty_sweep_summary.csv",
        list(summary_rows[0].keys()),
        summary_rows,
    )


def current_best_valid_energy(
    run_rows: Sequence[Mapping[str, object]],
) -> float:
    """Return the best valid energy already checkpointed."""
    energies = [
        float(row["best_energy"])
        for row in run_rows
        if parse_bool(row["valid_best_solution"])
    ]
    return min(energies, default=math.inf)
...
def main() -> None:
    ...
    runs_path = OUTPUT_DIR / "qpu_penalty_sweep_runs.csv"
    all_run_rows = load_checkpoint_rows(runs_path)

    completed = {
        (round(float(row["assignment_penalty"]), 12), int(row["run"]))
        for row in all_run_rows
    }

    print("\nDIRECT-QPU ASSIGNMENT-PENALTY SWEEP — RESUMABLE")
    print("=" * 62)
    ...
    print(f"Independent runs per penalty: {runs_per_penalty}")
    print(f"Reads per run: {NUM_READS}")
    print(f"Previously completed runs found: {len(all_run_rows)}")
    print(
        "Held fixed: normalization, congestion weight, pruning, "
        "chain-strength prefactor, spin reversals, annealing time, and reads."
    )

    # Build the QPU/composite stack ONCE for the entire process.
    sampler, qpu = create_qpu_sampler()
    ...
    for penalty_index, (assignment_penalty, penalty_source) in enumerate(
        ...
        for run_number in range(1, runs_per_penalty + 1):
            key = (round(assignment_penalty, 12), run_number)
            ...
            sampleset, reads_per_transform = solve_with_qpu(
                bqm,
                sampler=sampler,
            ...
            best_chain_break = float(
                getattr(result["best"], "chain_break_fraction", 0.0)
            )

            timing = sampleset.info.get("timing", {}) or {}
            qpu_access_time = float(
                timing.get("qpu_access_time", 0.0) or 0.0
            )
            qpu_sampling_time = float(
                timing.get("qpu_sampling_time", 0.0) or 0.0
            )
            ...
            run_row = {
                ...
                "best_chain_break_fraction": best_chain_break,
                "solver_wall_clock_seconds": solver_seconds,
                "qpu_access_time_microseconds": qpu_access_time,
                "qpu_sampling_time_microseconds": qpu_sampling_time,
            }

            # Compare against the checkpoint BEFORE adding this row.
            previous_best = current_best_valid_energy(all_run_rows)

            all_run_rows.append(run_row)
            completed.add(key)

            # CHECKPOINT AFTER EVERY SINGLE QPU JOB.
            write_penalty_sweep_outputs(all_run_rows)

            # Whenever a new global best valid solution appears, immediately
            # persist the normal detailed outputs too.
            if result["valid"] and float(result["best"].energy) < previous_best:
                export_bqm_csvs(
                    bqm,
                    routes,
                    distances,
                ...
                write_solution_outputs(
                    ...
                    result["total_people"],
                    result["congestion_objective"],
                    result["overload_objective"],
                    result["edge_report"],
                    result["exit_report"],
                    assignment_penalty,
                    objective_scales,
            ...
            print(
                ...
                f"chain_break={100.0 * best_chain_break:.2f}%, "
                f"time={solver_seconds:.3f}s"
            )

            # Release the large per-run objects before the next QPU job.
            del result
            del sampleset
            gc.collect()

        # Also collect between penalty values.
        del bqm
        gc.collect()

    write_penalty_sweep_outputs(all_run_rows)

    print("\nASSIGNMENT-PENALTY SWEEP COMPLETE")
    print("=" * 62)
    print(f"Completed run count:          {len(all_run_rows)}")
    print(
        "Per-run checkpoint:          "
        f"{OUTPUT_DIR / 'qpu_penalty_sweep_runs.csv'}"
    )
    print(
        "Per-penalty summary:         "
        f"{OUTPUT_DIR / 'qpu_penalty_sweep_summary.csv'}"
    )

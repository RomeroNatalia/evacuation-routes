...
Using QUBO Optimization and Quantum Annealing} \\ \large (Draft v4)}
...
\date{Draft v4 --- August 2026}

\begin{document}
\maketitle

\section*{Revision Notes (v3 $\to$ v4)}
\label{sec:revision-notes-v4}

v3 (\path{paper/main.tex}) disclosed, but did not fix, two open issues in the synthetic-corpus analysis (Section~\ref{sec:synthetic}): (i) whether the pruned-vs-unpruned energy bug already found and fixed for the 5-benchmark-floorplan comparison (Table~\ref{tab:milpgaps}) also affected the 10-repeat synthetic-corpus dataset, and (ii) that the Loop and Tree topologies' generator produced some short diagonal connector edges rather than pure axis-aligned corridors.

v4 resolves both, independently and for free (no D-Wave Leap quota spent):

\textbf{Pruned-vs-unpruned (confirmed negligible).} Recomputing the true unpruned energy from each winning Neal assignment across all 18 (regenerated) synthetic floorplans, the difference from the pruned-BQM search energy is at most 0.68 percentage points -- consistent with the spot-check already in v3's Limitations (max 0.26\% on 3 floorplans) and the original 5-floorplan figure ($<0.4\%$). This bug does not change any reported conclusion.

\textbf{Diagonal edges (a real effect, now fixed).} A diagonal edge in the generator moves both in $x$ and $y$ in a single hop, while every edge is hardcoded to \texttt{distance=1.0} regardless of angle -- so a diagonal edge was a genuine one-hop shortcut a real, wall-respecting corridor could not take (going around the corner would take two axis-aligned hops). This affected Loop's door-to-corridor edge (one per room) and ring-to-exit edge (one per exit), and Tree's spine-to-first-branch-node edge (one per branch); Linear was unaffected. Fixed by routing each through an axis-aligned elbow node instead, and regenerated all 18 synthetic floorplans under new \texttt{\_V2} IDs (Section~\ref{sec:v4-diagonal}), leaving the original 18 untouched for direct comparison.

\textbf{What this changes.} Loop's and Tree's measured optimality gaps drop substantially once the shortcut is removed (Loop: up to $-21$ percentage points at 30 rooms; Tree: up to $-6.65$pp), while Linear -- which never had diagonal edges -- is essentially unchanged, confirming the effect is real and not solver noise. The paper's central qualitative finding survives, and if anything strengthens: corridor diameter remains exactly tied between Tree and Loop at matched room counts after the fix, and in the free (Neal-only) 23-instance correlation analysis, corridor diameter is now the strongest predictor among the structural metrics evaluated ($r=0.868$, overtaking variable count's $0.826$, itself down from v3's $0.865$), while bridge-count measures got weaker ($0.622 \to 0.457$). \textbf{[9.22.26]} A separate 12-instance sweep holding variable count exactly fixed (Section~\ref{sec:dsweep}) confirms this is not a size confound: optimality gap rises monotonically with corridor diameter alone under both solvers ($r=0.943$ Neal, $r=0.930$ Hybrid).

\textbf{Hybrid re-run: complete.} 180 Hybrid calls (10 repeats $\times$ 18 V2 floorplans) were run on the diagonal-fixed corpus. Hybrid's mean gap is lower than Neal's in 17 of 23 instances and Bonferroni-significant (Mann-Whitney $U$, $\alpha=0.05/23$) in 11 of 23, both lower and significant in 10 of 23 -- essentially unchanged from v3's 18/23, 16/23, 11/23, confirming the Hybrid-outperforms-Neal finding was not an artifact of the diagonal-edge bug. The correlation and gap tables below now include both solvers. The paper's main tables (Tables~\ref{tab:correlations}, \ref{tab:synthetic}, \ref{tab:appendixb2}) still show the v3/original-corpus numbers as originally published; Section~\ref{sec:v4-diagonal} is where the V2-corrected numbers live for this draft.

\begin{abstract}
This paper formulates static indoor evacuation-route assignment as a capacity-aware Quadratic Unconstrained Binary Optimization (QUBO) problem, solves it with classical simulated annealing, D-Wave Leap Hybrid, and direct quantum annealing across five manually constructed benchmark building layouts (16 to 264 logical variables), and then goes further than prior work of this kind in three ways. First, we formulate the same constrained objective in Google OR-Tools CP-SAT using a scaled-integer representation. CP-SAT reaches status \texttt{OPTIMAL} for every floorplan tested in under two seconds, and the returned assignments are rescored independently under the original unpruned float64 objective. This reference baseline reveals that Neal and D-Wave Hybrid, previously reported as producing good solutions, are approximately 94--125\% above the reference energy on two of the five floorplans, a gap invisible without an exact solver-side optimum for the scaled formulation. Second, we investigate why: a controlled compute-budget experiment (up to 100$\times$ the original sweep count) fails to close the gap, ruling out under-sampling as the sole explanation, while a structural/geometric analysis and an 18-floorplan synthetic corpus with independently controlled corridor topology (linear, branching, and looped) identify \emph{corridor diameter} -- not bridge/bottleneck-edge count, our initial hypothesis -- as a predictor comparably strong to raw problem size ($r\approx0.83$) that nonetheless captures independent information, adding up to 13 additional percentage points of explained variance beyond variable count alone in a joint regression. This finding replicates under both classical and hybrid quantum solvers with proper significance testing (Mann-Whitney $U$, Bonferroni-corrected across 23 floorplans). Third, we report formal significance tests throughout, correcting the descriptive-statistics-only limitation of the original study. The result is a fully reproducible evacuation-QUBO workflow with a substantially stronger classical reference baseline than prior work in this line, and an empirically grounded, statistically confirmed account of what makes a floorplan hard for current annealing-based solvers. \textbf{[v4]} A geometry bug in the synthetic-corpus generator -- diagonal connector edges acting as uncredited one-hop shortcuts in the Loop and Tree topologies -- is identified, fixed, and the corpus regenerated; both Neal (free) and Hybrid (180 calls) were re-run on the corrected corpus. Measured optimality gaps drop substantially for the affected topologies under both solvers (Loop: up to $-21$pp Neal, $-19$pp Hybrid), while the paper's central structural finding survives and strengthens under both: corridor diameter becomes the single strongest predictor of solver difficulty for Neal ($r=0.868$) and Hybrid ($r=0.853$), ahead of raw variable count for both. The Hybrid-outperforms-Neal finding itself is essentially unchanged (11 of 23 both lower and Bonferroni-significant, vs.\ 11 of 23 originally).
\end{abstract}
...
\item \textbf{New:} a CP-SAT scaled-integer reformulation solved to status \texttt{OPTIMAL} for all five floorplans -- not only the smallest -- and all 18 synthetic instances, with each returned assignment rescored under the original unpruned objective, exposing optimality gaps of 94--125\% on two floorplans that a solver-vs-solver comparison alone cannot reveal.
...
\item \textbf{New:} an 18-floorplan synthetic corpus with independently controlled corridor topology, used to test the structural hypothesis under conditions the original five floorplans cannot provide, statistically confirming that corridor diameter (not bridge count, our initial hypothesis) is the strongest predictor among the structural metrics evaluated, ahead of raw variable count, replicated under both Neal and Hybrid (Section~\ref{sec:v4-diagonal}).
\item \textbf{[9.22.26] New:} a 12-instance deconfounding sweep (Section~\ref{sec:dsweep}) that fixes variable count exactly and varies corridor diameter alone, confirming the diameter effect is not an artifact of diameter's correlation with variable count in the main synthetic corpus.
\item \textbf{[9.22.26] New:} logical QUBO-interaction-graph metrics (Section~\ref{sec:qubograph}) computed directly on the BQM the solvers search, showing that first-order graph statistics (degree, density, interaction count) do not out-predict corridor diameter once variable count is controlled for.
\item \textbf{[9.22.26] New:} a mixed-effects model (Section~\ref{sec:mixedeffects}) with a random intercept per unique graph structure (14 groups, not 23 pooled instances), confirming corridor diameter remains a significant predictor beyond variable count under proper clustering.
\end{itemize}
...
We use Google OR-Tools CP-SAT \cite{ortools}, an open-source, license-free constraint-programming solver -- not an MIQP solver in the general sense, but sufficient for this integer-coefficient instance -- so the reference baseline is independently reproducible without any commercial solver license. Because CP-SAT requires integer coefficients, normalized load coefficients are scaled and rounded before optimization. CP-SAT's \texttt{OPTIMAL} certificate therefore applies directly to this integer-scaled formulation. The selected assignment is then recomputed independently in float64 using the identical unpruned objective formula in Equations~3--5. This rescoring reports the original-objective energy of the CP-SAT-selected assignment, but by itself does not prove that coefficient rounding could never change assignment ordering under the unrounded objective; we therefore treat these values as a strong exact-solver reference rather than claiming a formal certificate for the original floating-point objective. The Validation and Scaling-stability checks below build confidence in the reformulation, rescoring, and empirical (though not formally proven) order-preservation under rounding (Section~\ref{sec:limitations}).
...
Every one of the five benchmark floorplans, and all 18 synthetic floorplans introduced in Section~\ref{sec:synthetic}, solved to CP-SAT status \texttt{OPTIMAL} for the integer-scaled formulation in under two seconds each.
...
\label{sec:gaps}
Table~\ref{tab:milpgaps} reports each floorplan's CP-SAT reference energy alongside the original 30-run best-observed energies. FP04's CP-SAT reference matches the previously-reported exact value exactly. FP01 and FP05, both embeddable on the QPU, show modest gaps (3--14\%). FP02 and FP03 -- precisely the two floorplans that could not be directly embedded on the QPU -- show Neal and Hybrid landing \textbf{94--125\%} above the CP-SAT reference energy: more than double the reference on the hardest cases, a gap invisible in Table~\ref{tab:meanquality}'s solver-vs-solver comparison, where FP02 and FP03 do not stand out as unusual at all.

\begin{table}[h]
\centering
\caption{CP-SAT reference energy vs. best-observed energy (30-run benchmark), with relative gap. Neal/Hybrid values are the unpruned objective (Equations~3--5) recomputed from the winning assignment, matching the CP-SAT reference's basis exactly -- not the pruned-BQM search energy, which differs by a small ($<0.4\%$) amount.}
\label{tab:milpgaps}
\small
...
For FP03 specifically, the practical severity of this gap is best appreciated physically: the CP-SAT reference assignment (distance 254.25, raw congestion 11.85) is barely worse than the trivial, congestion-blind nearest-exit-Dijkstra baseline (distance 250.75, raw congestion 64.49, Appendix~A), while Neal and Hybrid's reported "100\% valid" solutions (distance 551--595, raw congestion 134--148) are simultaneously worse than that trivial baseline on \emph{both} distance and congestion -- a strictly dominated solution that a validity check alone cannot detect, since validity only verifies the exactly-one constraint, not solution quality.

\subsection{Statistical significance}
\label{sec:significance}
The original study's Section~7 explicitly notes the absence of formal significance testing. We apply Mann-Whitney $U$ (not paired Wilcoxon: Neal's local seeds and Hybrid's server-side randomness are independent, not paired samples) to the 30-run distributions, Bonferroni-corrected across five floorplans ($\alpha=0.01$). Hybrid is significantly better than Neal on FP01, FP03, and FP05 ($p<10^{-5}$) and on FP02 ($p\approx1.02\times10^{-5}$, significant after Bonferroni correction though just above the $10^{-5}$ mark); FP04 shows a technically significant but practically meaningless $p=1.7\times10^{-14}$, reflecting floating-point-noise-level differences between two distributions that both reach the exact optimum on every run -- a concrete illustration of why $p$-values must be read alongside effect size, not alone. Per-run (not just best-run) optimality gaps against the CP-SAT reference show neither solver ever reaches the reference energy across 30 independent runs on FP02 or FP03 (closest approach: Neal 124.76\%, Hybrid 120.82\% on FP02).

\subsection{Root-cause investigation: ruling out compute budget}
\label{sec:budget}
FP02 and FP03's large gaps could plausibly reflect an under-provisioned, fixed compute budget (1,000 reads, 5,000 sweeps for every floorplan regardless of its 16-to-264-variable range) rather than any structural property. We test this directly: rerunning Neal on FP02 and FP03 with 5$\times$, 20$\times$, and 100$\times$ the original sweep count (up to 500,000 sweeps), at 200 total reads per setting (50 reads across each of four seeds) rather than the original benchmark's 1,000 -- so this ablation tests sensitivity to sweep count specifically, not a controlled like-for-like increase in total compute budget. The gap does \emph{not} close -- both floorplans oscillate in roughly the same range regardless of sweep count (FP02: 153--163\%; FP03: 110--161\%, non-monotonically), ruling out under-sampling via sweep count alone as the explanation and reframing the question from \emph{how much} compute is needed to \emph{what about the landscape traps the search} in the first place.
...
FP02 and FP03 have roughly 2--25$\times$ more bridges and critical bridges than FP01 and FP04, and the most capacity-bottleneck edges by a wide margin (Table~\ref{tab:geometry}); against FP05 specifically, the gap is far narrower (1.3--1.5$\times$), so bridge count alone does not cleanly separate FP02/FP03 from every other floorplan. FP03 (dormitory) has a corridor-subgraph diameter of 40 hops -- roughly double every other floorplan -- consistent with a long single-spine hallway layout.
...
Repeating this analysis with ten independent Neal \emph{and} ten independent Hybrid runs on all 23 floorplans (230 Hybrid calls total) allows both a direct significance test and a check that the structural finding is not an artifact of Neal's particular search dynamics. Hybrid's mean gap is smaller than Neal's in 18 of 23 floorplans. Solver distributions differ significantly (Bonferroni-corrected across 23 tests) in 16 of 23 floorplans -- not necessarily the same 16 where Hybrid was better; Hybrid is both lower in mean gap \emph{and} Bonferroni-significant in 11 of 23. This still extends the original study's Hybrid-outperforms-Neal finding to the full synthetic corpus, with the more precise figure being the 11/23 both-and count. Corridor diameter predicts Hybrid's gap ($r=0.823$, $p=1.4\times10^{-6}$) essentially as well as it predicts Neal's ($r=0.837$, $p=6.3\times10^{-7}$): the structural mechanism is not solver-specific.

\subsection{\textbf{[v4]} Correcting a geometry artifact: diagonal shortcut edges in the synthetic corpus}
\label{sec:v4-diagonal}

\begin{quote}
\textbf{Status: complete.} Both Neal (free, local) and Hybrid (180 calls, using the corresponding author's own D-Wave Leap account) have been re-run on the diagonal-fixed corpus.
\end{quote}

Every edge \path{scripts/expansion/generate_synthetic_floorplan.py} creates is assigned \texttt{distance=1.0}, regardless of the angle it is drawn at. This is harmless for an axis-aligned edge, but three sites produced edges that moved diagonally -- both in $x$ and $y$ in a single hop: Loop's door-to-corridor edge (one per room), Loop's ring-to-exit edge (one per exit), and Tree's spine-to-first-branch-node edge (one per branch). A real, wall-respecting corridor cannot cut a corner this way; reaching the same point axis-aligned requires two hops, not one. Every such edge was therefore a one-hop shortcut not available to the five manually constructed benchmark layouts, whose graphs are hand-built to be wall-aware. Linear's topology never produced a diagonal edge and is an unaffected control.

Fixed by routing each affected connection through an intermediate axis-aligned elbow node (turn the corner in two hops instead of cutting through it), verified programmatically afterward that zero diagonal edges remain in any of the three topologies, and regenerated all 18 synthetic floorplans under new \texttt{\_V2} identifiers (e.g.\ \path{SYN_LOO_20_S1_V2}), leaving the original 18 untouched on disk for direct comparison.

\subsubsection{Structural metrics shift for Loop and Tree, not Linear}

\begin{table}[h]
\centering
\caption{Corridor diameter and route hop length, original synthetic corpus vs.\ diagonal-fixed V2, seed S1 (S2 moves identically since structure is seed-independent).}
\label{tab:v4-structural}
\small
\begin{tabular}{lrrrr}
\toprule
Floorplan & Corridor diam.\ (old $\to$ V2) & Route hops mean (old $\to$ V2) & Bridges (old $\to$ V2) \\
\midrule
Linear-10 & $9 \to 9$ & $7.5 \to 7.5$ & $31 \to 31$ \\
Linear-20 & $19 \to 19$ & $10.6 \to 10.6$ & $63 \to 63$ \\
Linear-30 & $29 \to 29$ & $13.7 \to 13.7$ & $95 \to 95$ \\
Loop-10 & $4 \to 6$ & $4.8 \to 6.8$ & $22 \to 34$ \\
Loop-20 & $8 \to 10$ & $6.9 \to 8.9$ & $44 \to 68$ \\
Loop-30 & $12 \to 14$ & $8.9 \to 10.9$ & $66 \to 102$ \\
Tree-10 & $8 \to 10$ & $7.3 \to 8.3$ & $37 \to 41$ \\
Tree-20 & $8 \to 10$ & $6.6 \to 7.6$ & $69 \to 77$ \\
Tree-30 & $10 \to 12$ & $7.6 \to 8.6$ & $105 \to 121$ \\
\bottomrule
\end{tabular}
\end{table}

Linear is byte-for-byte unaffected, as expected. Loop and Tree both gain exactly the elbow-node count the fix predicts (Loop: one per room plus one per exit; Tree: one per branch), confirmed directly against node/edge counts before recomputing anything else. Notably, Tree-20 and Loop-20's corridor diameter -- tied at 8 in the original corpus, the specific pairing Section~\ref{sec:synthetic} uses to argue bridge count doesn't order difficulty correctly -- remain \emph{exactly tied} after the fix, now at 10.

\subsubsection{Optimality gaps drop for Loop and Tree, Linear unchanged -- both solvers}

\begin{table}[h]
\centering
\caption{10-repeat mean optimality gap against the CP-SAT reference, original corpus vs.\ diagonal-fixed V2, both solvers (energies recomputed unpruned throughout).}
\label{tab:v4-gaps}
\small
\begin{tabular}{lrrrr}
\toprule
Floorplan & Old Neal $\to$ V2 Neal & $\Delta$ & Old Hybrid $\to$ V2 Hybrid & $\Delta$ \\
\midrule
Linear-20-S1 & $51.78 \to 51.79$ & $+0.01$ & $32.59 \to 35.96$ & $+3.37$ \\
Linear-20-S2 & $49.48 \to 49.52$ & $+0.04$ & $34.92 \to 32.64$ & $-2.28$ \\
Linear-30-S1 & $128.15 \to 128.83$ & $+0.68$ & $103.58 \to 99.17$ & $-4.41$ \\
Linear-30-S2 & $132.35 \to 133.03$ & $+0.68$ & $103.58 \to 102.09$ & $-1.49$ \\
Loop-20-S1 & $23.27 \to 17.32$ & $-5.95$ & $15.76 \to 10.53$ & $-5.23$ \\
Loop-20-S2 & $22.40 \to 16.76$ & $-5.64$ & $13.59 \to 11.46$ & $-2.13$ \\
Loop-30-S1 & $69.28 \to 48.16$ & $-21.12$ & $56.92 \to 38.03$ & $-18.89$ \\
Loop-30-S2 & $68.17 \to 48.75$ & $-19.42$ & $53.03 \to 37.39$ & $-15.64$ \\
Tree-20-S1 & $8.50 \to 6.15$ & $-2.35$ & $6.07 \to 3.93$ & $-2.14$ \\
Tree-20-S2 & $9.82 \to 7.20$ & $-2.62$ & $5.84 \to 4.57$ & $-1.27$ \\
Tree-30-S1 & $29.14 \to 22.49$ & $-6.65$ & $23.40 \to 17.01$ & $-6.39$ \\
Tree-30-S2 & $26.40 \to 22.42$ & $-3.98$ & $20.58 \to 18.10$ & $-2.48$ \\
\bottomrule
\end{tabular}
\end{table}

Linear moves within ordinary run-to-run noise under both solvers ($-2.28$ to $+3.37$pp), while Loop and Tree drop substantially and consistently under both, Loop far more than Tree (up to $-21.12$pp Neal / $-18.89$pp Hybrid vs.\ up to $-6.65$pp Neal / $-6.39$pp Hybrid), matching the asymmetry in how many diagonal shortcuts each topology had (one per room in Loop; one per branch in Tree). The same pattern appearing under two independently-run solvers is strong evidence the diagonal-edge artifact was inflating measured difficulty specifically for the topologies that had it, not solver-specific noise.

\subsubsection{The core finding survives, and strengthens -- both solvers}

Combining the unchanged 5 benchmark floorplans with the diagonal-fixed 18-floorplan V2 corpus ($n=23$) and repeating the correlation analysis behind Table~\ref{tab:correlations}:

\begin{table}[h]
\centering
\caption{Predictor correlation with 10-repeat mean gap, original 23-instance corpus (Table~\ref{tab:correlations}) vs.\ V2 diagonal-fixed 23-instance corpus, both solvers.}
\label{tab:v4-correlations}
\small
\begin{tabular}{lrrrr}
\toprule
Predictor & Old Neal $r$ & V2 Neal $r$ & Old Hybrid $r$ & V2 Hybrid $r$ \\
\midrule
n\_variables & 0.865 & 0.826 & 0.873 & 0.836 \\
corridor\_diameter\_hops & 0.837 & \textbf{0.868} & 0.823 & \textbf{0.853} \\
route\_hops\_mean & 0.736 & 0.765 & 0.761 & 0.800 \\
capacity\_bottleneck\_edge\_count & 0.702 & 0.712 & 0.743 & 0.763 \\
traffic\_bottleneck\_top1\_share & 0.713 & 0.697 & 0.699 & 0.680 \\
structural\_critical\_bridge\_count & 0.702 & 0.586 & 0.700 & 0.585 \\
structural\_bridge\_count & 0.622 & 0.457 & 0.603 & 0.436 \\
\bottomrule
\end{tabular}
\end{table}

Corridor diameter is now the strongest predictor among the structural metrics evaluated, for \emph{both} solvers, ahead of raw variable count in both cases -- the opposite direction from a concern that removing an artifact might weaken the finding. This is still a correlational result at this stage, since variable count and corridor diameter both vary together across the 23-instance corpus; Section~\ref{sec:dsweep} addresses that confound directly with a controlled sweep. The bridge-based measures, already the weakest predictors and already rejected as the primary explanation in Section~\ref{sec:synthetic}, get weaker still, again for both solvers. The Neal regression result: $R^2$ from variable count alone is $0.682$ (V2) vs.\ $0.749$ (old); adding corridor diameter raises it to $0.857$ (V2) vs.\ $0.882$ (old); partial correlation $r=0.742$, $p=5.0\times10^{-5}$ (V2) vs.\ $r=0.727$, $p=8.5\times10^{-5}$ (old).

\subsubsection{Hybrid-outperforms-Neal finding: essentially unchanged}

Repeating Section~\ref{sec:synthetic}'s Mann-Whitney significance test ($\alpha=0.05$, Bonferroni-corrected across 23 instances) on the V2 corpus: Hybrid's mean gap is lower than Neal's in \textbf{17 of 23} instances (old: 18 of 23), Bonferroni-significant in \textbf{11 of 23} (old: 16 of 23), and both lower \emph{and} significant in \textbf{10 of 23} (old: 11 of 23). The headline comparative claim is essentially preserved -- within one instance of the original counts on every measure -- confirming the diagonal-edge bug affected absolute gap magnitude and the structural-predictor analysis, but not the solver-vs-solver comparison itself.

\subsubsection{[9.22.26] Deconfounding corridor diameter from variable count}
\label{sec:dsweep}
...
At the univariate level, several logical-QUBO-graph metrics correlate with optimality gap about as strongly as corridor diameter does -- unsurprising, since larger problems mechanically have more possible interactions. The informative result is the partial correlation controlling for $N$: degree and density collapse to statistically indistinguishable from zero once variable count is controlled for, while corridor diameter retains a strong, significant partial correlation ($r=0.742$ Neal, $r=0.707$ Hybrid, matching the values already reported in Section~\ref{sec:v4-diagonal}). In other words, at this level of analysis -- first-order interaction-graph statistics -- the logical QUBO graph does not explain the gap any better than the navigation graph's corridor diameter already does; if anything, these simple graph-level metrics turn out to be largely a proxy for $N$ once diameter's own $N$-independence (Section~\ref{sec:dsweep}) is accounted for. This is consistent with, not contradictory to, the corridor-diameter finding: it does not identify a \emph{better} predictor at the QUBO-graph level, but it does rule out the concern that a trivial graph-density statistic on the logical BQM would have explained the gap better than the physical corridor structure did. Deeper QUBO-landscape properties -- treewidth, spectral gap, frustration, ground-state degeneracy -- remain outside this analysis's scope and are noted as future work.

\subsubsection{[9.22.26] Accounting for repeated graph structure: a mixed-effects check}
\label{sec:mixedeffects}

The 23-instance correlations above (Sections~\ref{sec:v4-diagonal},~\ref{sec:qubograph}) treat every floorplan as an independent observation, but the 18-floorplan synthetic corpus contains only 9 unique topology/size graph structures, each evaluated under two independently-drawn occupancy/capacity seeds -- a limitation already flagged in Section~\ref{sec:limitations}. Combined with the five uniquely-structured benchmark floorplans, the corpus has \textbf{14 unique graph structures, not 23}. We fit the model the confound calls for directly: $\text{gap}_{ij} = \beta_0 + \beta_1 \cdot \text{diameter}_i + \beta_2 \cdot n\_\text{variables}_i + u_i + \varepsilon_{ij}$, with a random intercept $u_i$ per unique graph structure (14 groups), using \texttt{statsmodels}' \texttt{MixedLM}.

Corridor diameter remains a significant predictor after this correction, for both solvers: Neal coefficient $2.38$ ($p=0.002$), Hybrid coefficient $1.84$ ($p=0.006$), both alongside a significant $n\_\text{variables}$ term. A likelihood-ratio test comparing this model against the reduced model without diameter confirms diameter adds explanatory power beyond $n\_\text{variables}$ even under proper clustering (Neal: $\text{LR}=7.25$, $p=0.007$; Hybrid: $\text{LR}=5.94$, $p=0.015$). This addresses the repeated-structure concern directly rather than only via the informal 14-unique-instance spot-check already reported in Section~\ref{sec:limitations} (correlation unchanged to three decimal places when restricted to unique structures): even a formal random-intercept model that does not discard the seed-level observations still finds diameter significant.

\subsection{Direct-QPU feasibility and embedding}
Direct-QPU performance remains strongly instance-dependent, retained from the original study: FP04 reproduced the exact optimum with 78.78\% mean valid-sample rate; FP01 and FP05 required chains up to 9 and 12 physical qubits per logical variable respectively, with valid-sample rates below 1\%; FP02 and FP03 could not be embedded at all. Given Section~\ref{sec:milp}'s finding that the same two floorplans are also the hardest for \emph{classical} optimization once measured against the CP-SAT reference, embedding difficulty and solution-quality difficulty for this formulation appear to share a common structural origin (corridor diameter) rather than being independent failure modes of the quantum-specific pipeline.

\section{Discussion}

\subsection{A strong reference baseline changes the empirical picture, not just its precision}
The original study's central comparative claim -- Hybrid outperforms Neal -- survives and is now statistically confirmed. But the \emph{absolute} quality claim it could not make (how good are these solutions, really?) changes substantially once a CP-SAT reference exists: two of five floorplans were, in fact, roughly twice the reference energy in the best saved annealing assignments, a finding a solver-vs-solver comparison structurally cannot surface, since both solvers can simultaneously be far from a stronger classical reference while one still beats the other. This is a general methodological point beyond this specific evacuation formulation: reporting only relative solver comparisons on QUBO-formulated problems, without an exact or reference baseline wherever the underlying combinatorial structure permits one, risks reporting precise but potentially misleading conclusions about absolute solution quality.
...
\item CP-SAT certifies the integer-scaled reformulation used for optimization. The returned assignment is rescored under the original float64 objective; a scaling-stability check (Section~\ref{sec:scalingstability}) finds the selected assignment and recomputed energy byte-identical across four orders of magnitude of coefficient-rounding granularity for all five benchmark floorplans and the corpus's worst-case synthetic instance, which is strong empirical evidence against a rounding artifact -- but this is not a formal proof that coefficient rounding preserves assignment ordering on every instance.
\item A strong CP-SAT reference baseline now exists for all 23 floorplans studied (five benchmark, 18 synthetic) -- resolving the original study's single-instance-only limitation -- but the benchmark-floorplan corpus remains five manually constructed layouts, five building types; the synthetic corpus's controlled topologies are not a substitute for a larger, independently-sourced real-building sample.
...
\item \textbf{\textbf{[v4 -- confirmed, Neal side]} Unlike Table~\ref{tab:milpgaps}, the 10-repeat Neal/Hybrid energies behind Tables~\ref{tab:correlations}, \ref{tab:synthetic}, and \ref{tab:appendixb2} (Section~\ref{sec:synthetic}) are the pruned-BQM search energy, not the independently-recomputed unpruned objective.} Section~\ref{sec:v4-diagonal} recomputed the true unpruned energy directly (not just spot-checked) across all 18 regenerated floorplans' Neal runs: maximum difference 0.68 percentage points, consistent with the original spot-check (max 0.26\%) and the 5-floorplan figure ($<0.4\%$). Confirmed negligible for Neal; the Hybrid side (230 calls) has still not been recomputed, since doing so costs D-Wave Leap quota.
\item \textbf{\textbf{[v4 -- resolved]} The synthetic generator's Loop and Tree topologies included some short non-axis-aligned (diagonal) connector edges that were, in fact, genuine one-hop shortcuts (every edge is weighted \texttt{distance=1.0} regardless of angle, so a diagonal hop covered more net displacement than an axis-aligned one could in a single hop).} Root-caused, fixed (routed through an axis-aligned elbow node instead), and the corpus regenerated as \texttt{\_V2} (Section~\ref{sec:v4-diagonal}); zero diagonal edges remain, verified programmatically. Both Neal and Hybrid were re-run on the corrected corpus (180 Hybrid calls); this measurably changed structural metrics and both solvers' gaps for Loop and Tree (Tables~\ref{tab:v4-structural}, \ref{tab:v4-gaps}), while the corridor-diameter finding and the Hybrid-outperforms-Neal finding both survive. The paper's main tables (Tables~\ref{tab:correlations}, \ref{tab:synthetic}, \ref{tab:appendixb2}) still show the originally-published, diagonal-affected numbers; Section~\ref{sec:v4-diagonal} is where the corrected numbers currently live pending a decision on whether to replace them outright in a future version.
...
\textbf{[v4 -- done]} The diagonal-edge correction (Section~\ref{sec:v4-diagonal}) is now complete for both solvers. The remaining decision is whether to replace Tables~\ref{tab:correlations}, \ref{tab:synthetic}, and \ref{tab:appendixb2} outright with the V2 numbers in a future version, versus keeping both corpora side by side as this draft does.

The next-highest-priority extension, retained from the original study, remains empirical and simulation-based validation of the capacity model against a validated evacuation simulator or controlled pedestrian studies. Beyond that, this revision's findings motivate: (i) a formal order-preservation proof, exact rational formulation, or another exact continuous-coefficient method to complement the empirical scaling-stability check already performed (Section~\ref{sec:scalingstability}); (ii) a synthetic sweep varying corridor diameter independently of route-hop count specifically (variable count is already deconfounded from diameter, Section~\ref{sec:dsweep}), to isolate the remaining diameter/route-length pairing; (iii) deeper QUBO-landscape properties beyond the first-order interaction-graph statistics already computed (Section~\ref{sec:qubograph}) -- treewidth, spectral gap, frustration, and ground-state degeneracy -- which may identify a mechanism the navigation graph's corridor diameter does not fully capture; (iv) a systematic embedding study across the full synthetic corpus (chain length, chain-break rate, embedding success probability as a function of corridor diameter), extending the five-floorplan direct-QPU results in Section~\ref{sec:v4-diagonal}'s predecessor discussion to test whether embedding difficulty and solution-quality difficulty share a common structural origin, as their co-occurrence on FP02/FP03 alone suggests but cannot establish; (v) applying the same reference-baseline methodology to other QUBO-formulated routing problems in the literature reviewed in Section~2, to test whether the same embedding-difficulty/solution-quality correlation generalizes beyond evacuation routing; (vi) an independently-sourced real-building corpus (e.g.\ the Modified Swiss Dwellings dataset \cite{msd}, considered but not used in the original study) annotated with the same wall-aware graph pipeline, to test whether corridor diameter predicts difficulty on buildings this project did not design; and (vii) dynamic hazard layers connecting the static assignment framework to risk-aware routing while preserving the graph-validation pipeline.

\section{Conclusion}

This paper extends a capacity-aware indoor evacuation-route assignment QUBO study, originally reporting best-observed solver energies across 390 runs on five floorplans, with a strong CP-SAT reference baseline, formal significance testing, and a controlled structural investigation. Reformulating the QUBO's penalty-method constraint as a native linear equality reveals the underlying problem is convex, letting a general-purpose, license-free CP-SAT solver reach status \texttt{OPTIMAL} for the integer-scaled formulation of every floorplan -- including the two too dense to embed on the QPU -- in under two seconds each. Measured against these reference energies, Neal and D-Wave Hybrid are roughly twice the reference energy on exactly those two floorplans, a finding invisible in the original solver-vs-solver comparison. A direct compute-budget ablation rules out under-sampling as the sole explanation; a controlled 18-floorplan synthetic corpus, built specifically to test and, ultimately, correct our own initial structural hypothesis, statistically confirms that corridor diameter -- not bridge/bottleneck-edge count -- is the strongest predictor among the structural metrics evaluated, ahead of raw variable count, replicated under both classical and hybrid quantum solvers with Bonferroni-corrected Mann-Whitney testing. A further 12-instance sweep holding variable count exactly fixed (Section~\ref{sec:dsweep}) confirms the diameter effect directly under both solvers: optimality gap rises monotonically with corridor diameter alone ($r=0.943$ Neal, $r=0.930$ Hybrid). The result is a reproducible evacuation-QUBO workflow with a substantially stronger classical reference than prior work in this line, and an empirically grounded, statistically confirmed, and self-corrected account of what makes a floorplan hard for current annealing-based solvers to route well.
...
\label{sec:appendixB}
Tables~\ref{tab:appendixb} and~\ref{tab:appendixb2} together list all 18 synthetic floorplans. Room occupancy is drawn uniformly from $[2,25]$ and edge capacities from small integer ranges matching the benchmark floorplans' scale (see the generator script in the accompanying repository, \path{scripts/expansion/generate_synthetic_floorplan.py}, for exact generation logic). Structural properties (Table~\ref{tab:appendixb}) are deterministic given topology and size, not affected by the random seed, so are listed once per topology-size combination; CP-SAT reference energies and solver gaps (Table~\ref{tab:appendixb2}) vary by seed (occupancy/capacity draw) and are listed for both.
...
\begin{table}[h]
\centering
\caption{Synthetic corpus full results, all 18 floorplans (both seeds): CP-SAT reference energy and 10-repeat mean optimality gap for each solver.}
...
\end{subfigure}
\caption{The five benchmark floorplans (Table~\ref{tab:floorplans}). FP03's long single corridor spine and FP02's dense multi-wing layout are visually apparent here, consistent with their large reference-energy gaps (Section~\ref{sec:gaps}).}
...
\end{subfigure}
\caption{The 9 unique synthetic layouts (rows: Linear, Tree, Loop; columns: 10, 20, 30 rooms), representative of both seeds each. Compare against Table~\ref{tab:appendixb}'s structural metrics: Linear's single spine, Tree's dual-row branch structure, and Loop's ring are all visible directly. \textbf{Note (v4): these are the original, diagonal-affected blueprints}, retained for comparison against Figure~\ref{fig:blueprints-v2} below -- look closely at Loop and Tree and the diagonal door/ring-to-exit connectors described in Section~\ref{sec:v4-diagonal} are visible directly.}
...
\end{subfigure}
\caption{\textbf{[v4]} The same 9 layouts after the diagonal-edge fix (Section~\ref{sec:v4-diagonal}). Compare directly against Figure~\ref{fig:blueprints-synthetic}: Loop's door-to-corridor and ring-to-exit connections, and Tree's spine-to-first-branch connection, now route through a right-angle elbow instead of cutting the corner. Linear is visually identical to the previous figure, as expected. Verified programmatically (not just visually) that zero diagonal edges remain in any of the 18 V2 floorplans.}
\label{fig:blueprints-v2}
\end{figure}

\section{Reproducibility}
\textbf{[v4]} The diagonal-fixed synthetic corpus (18 floorplans, \path{*_V2} IDs) is at \path{data/floorplans/}, generated by the corrected \path{scripts/expansion/generate_synthetic_floorplan.py}. Its structural metrics are at \path{docs/expansion_synthetic_geometry_raw_V2.json}, CP-SAT reference energies at each floorplan's own \path{output/milp_gap/}, the Neal 10-repeat comparison (produced by \path{scripts/expansion/run_neal_repeats_v2_fixed.py}, which also fixes the pruned-vs-unpruned bug) at \path{docs/expansion_synthetic_neal_v2_fixed_raw.json}, and the Hybrid 10-repeat comparison (\path{scripts/expansion/run_hybrid_repeats_v2_fixed.py}) at \path{docs/expansion_synthetic_hybrid_v2_fixed_raw.json}. The combined 23-instance correlation dataset behind Table~\ref{tab:v4-correlations} is at \path{docs/expansion_v2_combined_23instance_raw.json}, and the Mann-Whitney significance results behind Section~\ref{sec:v4-diagonal}'s solver-comparison recount are at \path{docs/expansion_v2_significance_raw.json}.

\textbf{[9.22.26]} The scaling-stability check (Section~\ref{sec:scalingstability}) is at \path{scripts/expansion/scaling_stability_check.py}, with raw per-scale results at \path{docs/expansion_cpsat_scaling_stability_raw.json}. The diameter-deconfounding sweep (Section~\ref{sec:dsweep}) uses the extended generator (\path{scripts/expansion/generate_synthetic_floorplan.py}, \texttt{--corridor-length}) plus \path{scripts/expansion/diameter_sweep_geometry.py}, \path{scripts/expansion/diameter_sweep_neal.py}, and \path{scripts/expansion/diameter_sweep_hybrid.py}, with raw results at \path{docs/expansion_diameter_sweep_geometry_raw.json}, \path{docs/expansion_diameter_sweep_neal_raw.json}, and \path{docs/expansion_diameter_sweep_hybrid_raw.json}. The logical QUBO-interaction-graph metrics (Section~\ref{sec:qubograph}) are at \path{scripts/expansion/qubo_graph_metrics.py}, raw results at \path{docs/expansion_qubo_graph_metrics_raw.json}. The mixed-effects check (Section~\ref{sec:mixedeffects}, requires \texttt{statsmodels}) is at \path{scripts/expansion/mixed_effects_diameter_check.py}, raw results at \path{docs/expansion_mixed_effects_raw.json}. This revision's external review and response plan are tracked at \path{paper/revision/review_9.22.26.md} and \path{paper/revision/plan_9.22.26.md}.

Source code, all 23 floorplan datasets (five benchmark, 18 synthetic), generated route catalogs, validation tests, CP-SAT/Neal/Hybrid benchmark outputs, blueprint figures, and this manuscript's source are available at
\begin{center}

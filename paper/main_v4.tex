...
\begin{quote}
\textbf{Status: Neal-only, executed and free (no D-Wave Leap quota spent).} A Hybrid re-run on the corrected corpus is the remaining step; see the Revision Notes and Future Work.
\end{quote}
...
\subsubsection{Optimality gaps drop for Loop and Tree, Linear unchanged}

\begin{table}[h]
\centering
\caption{Neal 10-repeat mean optimality gap against the CP-SAT reference, original corpus vs.\ diagonal-fixed V2 (both energies recomputed unpruned, so this table is also net of the pruned-vs-unpruned bug).}
\label{tab:v4-gaps}
\small
\begin{tabular}{lrrr}
\toprule
Floorplan & Old Neal gap (\%) & V2 Neal gap (\%) & $\Delta$ (pp) \\
\midrule
Linear-20-S1 & 51.78 & 51.79 & $+0.01$ \\
Linear-30-S1 & 128.15 & 128.83 & $+0.68$ \\
Loop-20-S1 & 23.27 & 17.32 & $-5.95$ \\
Loop-20-S2 & 22.40 & 16.76 & $-5.64$ \\
Loop-30-S1 & 69.28 & 48.16 & $-21.12$ \\
Loop-30-S2 & 68.17 & 48.75 & $-19.42$ \\
Tree-20-S1 & 8.50 & 6.15 & $-2.35$ \\
Tree-20-S2 & 9.82 & 7.20 & $-2.62$ \\
Tree-30-S1 & 29.14 & 22.49 & $-6.65$ \\
Tree-30-S2 & 26.40 & 22.42 & $-3.98$ \\
\bottomrule
\end{tabular}
\end{table}

Linear moves by at most $0.68$ percentage points -- within ordinary run-to-run Neal variation -- while Loop and Tree both drop substantially and consistently across every affected instance, Loop far more than Tree (up to $-21.12$pp vs.\ up to $-6.65$pp), matching the asymmetry in how many diagonal shortcuts each topology had (one per room in Loop; one per branch in Tree). This is strong evidence the diagonal-edge artifact was inflating measured difficulty specifically for the topologies that had it, not a coincidence of solver noise.

\subsubsection{The core finding survives, and strengthens}

Combining the unchanged 5 benchmark floorplans with the diagonal-fixed 18-floorplan V2 corpus ($n=23$, Neal only) and repeating the correlation analysis behind Table~\ref{tab:correlations}:

\begin{table}[h]
\centering
\caption{Predictor correlation with Neal's 10-repeat mean gap, original 23-instance corpus (Table~\ref{tab:correlations}) vs.\ V2 diagonal-fixed 23-instance corpus.}
\label{tab:v4-correlations}
\small
\begin{tabular}{lrrr}
\toprule
Predictor & Old $r$ & V2 $r$ & V2 $p$ \\
\midrule
n\_variables & 0.865 & 0.826 & $1.22\times10^{-6}$ \\
corridor\_diameter\_hops & 0.837 & \textbf{0.868} & $7.93\times10^{-8}$ \\
route\_hops\_mean & 0.736 & 0.765 & $2.10\times10^{-5}$ \\
capacity\_bottleneck\_edge\_count & 0.702 & 0.712 & $1.38\times10^{-4}$ \\
traffic\_bottleneck\_top1\_share & 0.713 & 0.697 & $2.21\times10^{-4}$ \\
structural\_critical\_bridge\_count & 0.702 & 0.586 & $3.31\times10^{-3}$ \\
structural\_bridge\_count & 0.622 & 0.457 & $2.85\times10^{-2}$ \\
\bottomrule
\end{tabular}
\end{table}

Corridor diameter is now the single strongest predictor of the ten, ahead of raw variable count -- the opposite direction from a concern that removing an artifact might weaken the finding. The bridge-based measures, already the weakest predictors and already rejected as the primary explanation in Section~\ref{sec:synthetic}, get weaker still. The regression result is consistent: $R^2$ from variable count alone is $0.682$ (V2) vs.\ $0.749$ (old); adding corridor diameter raises it to $0.857$ (V2) vs.\ $0.882$ (old) -- an increase of $+0.175$ (V2) vs.\ $+0.133$ (old), and the partial correlation of corridor diameter controlling for variable count is $r=0.742$, $p=5.0\times10^{-5}$ (V2) vs.\ $r=0.727$, $p=8.5\times10^{-5}$ (old). Every headline number in Section~\ref{sec:synthetic} that this can be checked against moves in the direction of a \emph{stronger}, not weaker, corridor-diameter finding once the diagonal-edge artifact is removed.
...
\item \textbf{\textbf{[v4 -- fixed, Neal side]} The synthetic generator's Loop and Tree topologies included some short non-axis-aligned (diagonal) connector edges that were, in fact, genuine one-hop shortcuts (every edge is weighted \texttt{distance=1.0} regardless of angle, so a diagonal hop covered more net displacement than an axis-aligned one could in a single hop).} Root-caused, fixed (routed through an axis-aligned elbow node instead), and the corpus regenerated as \texttt{\_V2} (Section~\ref{sec:v4-diagonal}); zero diagonal edges remain, verified programmatically. This measurably changed structural metrics and Neal gaps for Loop and Tree (Tables~\ref{tab:v4-structural}, \ref{tab:v4-gaps}); the paper's main tables (Tables~\ref{tab:correlations}, \ref{tab:synthetic}, \ref{tab:appendixb2}), which use Hybrid as well as Neal, are not yet updated to the V2 corpus pending a Hybrid re-run.
...
\textbf{[v4]} The single highest-priority item is completing the correction started in Section~\ref{sec:v4-diagonal}: a Hybrid re-run (10 repeats $\times$ 18 V2 floorplans, ~180 calls, comparable quota cost to the original 230-call run) is the only thing separating the free Neal-only comparison in this draft from being able to officially replace Tables~\ref{tab:correlations}, \ref{tab:synthetic}, and \ref{tab:appendixb2} with the diagonal-corrected numbers. This is a deliberate pause for review, not a technical blocker -- see the accompanying repository discussion.
...
\textbf{[v4]} The diagonal-fixed synthetic corpus (18 floorplans, \path{*_V2} IDs) is at \path{data/floorplans/}, generated by the corrected \path{scripts/expansion/generate_synthetic_floorplan.py}. Its structural metrics are at \path{docs/expansion_synthetic_geometry_raw_V2.json}, CP-SAT reference energies at each floorplan's own \path{output/milp_gap/}, and the Neal 10-repeat comparison (produced by the new \path{scripts/expansion/run_neal_repeats_v2_fixed.py}, which also fixes the pruned-vs-unpruned bug) at \path{docs/expansion_synthetic_neal_v2_fixed_raw.json}. The combined 23-instance correlation dataset behind Table~\ref{tab:v4-correlations} is at \path{docs/expansion_v2_combined_23instance_raw.json}.

...
\item Unlike Table~\ref{tab:milpgaps}, the 10-repeat Neal/Hybrid energies behind Tables~\ref{tab:correlations}, \ref{tab:synthetic}, and \ref{tab:appendixb2} (Section~\ref{sec:synthetic}) are the pruned-BQM search energy, not the independently-recomputed unpruned objective. Spot-checks on representative synthetic floorplans (linear-30, tree-10, loop-20) show this differs by at most 0.26\% -- consistent with Table~\ref{tab:milpgaps}'s $<0.4\%$ figure -- so it should not change any reported correlation, gap, or significance result, but a full unpruned recomputation across all 230 Hybrid calls has not been performed.
\item The synthetic generator's Loop and Tree topologies include some short non-axis-aligned (diagonal) connector edges where a room or branch attaches at an angle; the benchmark floorplans were drawn with more attention to axis-aligned corridors. These are short, intentional connections rather than the long stray-edge defect fixed during development (Section~\ref{sec:synthetic}), but the generation process placed less emphasis on avoiding diagonal connectors than the manually constructed benchmark layouts did.

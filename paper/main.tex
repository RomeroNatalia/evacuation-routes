...
\author{
Arielle Jenna Fishman \and Natalia Romero, Ph.D. \\[4pt]
...
\item \textbf{New:} an exact convex-MIQP reformulation certifying the global optimum for all five floorplans -- not only the smallest -- solved in under two seconds each, exposing optimality gaps of 91--138\% on two floorplans that a solver-vs-solver comparison alone cannot reveal.
...
\label{sec:gaps}
Table~\ref{tab:milpgaps} reports each floorplan's certified global optimum alongside the original 30-run best-observed energies. FP04's certified optimum matches the previously-reported exact value exactly. FP01 and FP05, both embeddable on the QPU, show modest gaps (1--14\%). FP02 and FP03 -- precisely the two floorplans that could not be directly embedded on the QPU -- show Neal and Hybrid landing \textbf{91--110\%} above the certified optimum: roughly double the true minimum, a gap invisible in Table~\ref{tab:meanquality}'s solver-vs-solver comparison, where FP02 and FP03 do not stand out as unusual at all.
...
\midrule
FP01 & 0.593857 & 0.636840 & 7.24\% & 0.600250 & \textbf{1.08\%} \\
FP02 & 0.288830 & 0.585420 & 102.69\% & 0.575235 & \textbf{99.16\%} \\
FP03 & 0.260012 & 0.545110 & 109.65\% & 0.497328 & \textbf{91.27\%} \\
...
Combining all 23 floorplans (real and synthetic, $n=23$), every candidate structural predictor correlates significantly with Neal's optimality gap (all $p<0.002$). Corridor diameter is comparably strong to raw variable count as a univariate predictor -- marginally stronger for Neal's 10-repeat mean gap ($r=0.859$ vs.\ $0.851$ for variable count) and marginally weaker for Hybrid's ($r=0.849$ vs.\ $0.859$), always well ahead of critical-bridge count ($r=0.730$ and $0.729$ respectively). The interesting result is not that corridor diameter beats size outright, but that it is essentially \emph{as informative as size while capturing something size does not}, confirmed directly by the regression below. Critically, our original working hypothesis -- that critical-bridge count drives difficulty -- does not order the three controlled topologies correctly. At matched room counts, Tree has 4--5$\times$ more critical bridges than Loop (17 vs.\ 4 at 20 rooms; 30 vs.\ 6 at 30 rooms) yet a \emph{smaller} optimality gap both times (Table~\ref{tab:synthetic}). Corridor diameter and mean route length, by contrast, correctly order all three topologies (Tree $<$ Loop $<$ Linear) at every size tested.

\begin{table}[h]
\centering
\caption{Synthetic corpus: topology, structure, and 10-repeat mean optimality gap (Hybrid).}
\label{tab:synthetic}
\small
\begin{tabular}{lrrrrr}
\toprule
...
Multiple regression confirms corridor diameter adds substantial explanatory power beyond raw size: partial correlation controlling for variable count is $r=0.751$ ($p=3.6\times10^{-5}$), and $R^2$ rises from 0.724 (variable count alone) to 0.880 with corridor diameter added for Neal's 10-repeat mean gap (0.739 to 0.879 for Hybrid's) -- roughly five times the additional variance explained by critical-bridge count alone in the same regression framework.

\subsubsection{Replication under Hybrid, with proper significance testing}
Repeating this analysis with ten independent Neal \emph{and} ten independent Hybrid runs on all 23 floorplans (230 Hybrid calls total) allows both a direct significance test and a check that the structural finding is not an artifact of Neal's particular search dynamics. Hybrid's mean gap is smaller than Neal's in 18 of 23 floorplans, significantly so (Bonferroni-corrected across 23 tests) in 17 of 23 -- extending the original study's Hybrid-outperforms-Neal finding to the full synthetic corpus. Corridor diameter predicts Hybrid's gap ($r=0.849$, $p=3.0\times10^{-7}$) essentially as well as it predicts Neal's ($r=0.859$, $p=1.5\times10^{-7}$): the structural mechanism is not solver-specific.
...
\label{sec:appendixB}
Table~\ref{tab:appendixb} lists all 18 synthetic floorplans. Room occupancy is drawn uniformly from $[2,25]$ and edge capacities from small integer ranges matching the real floorplans' scale (see the generator script in the accompanying repository, \path{scripts/expansion/generate_synthetic_floorplan.py}, for exact generation logic); topology and, consequently, all structural metrics (bridges, corridor diameter, route length) are deterministic given topology and size, not affected by the random seed.

\begin{table}[h]
\centering
\caption{Synthetic corpus: 3 topologies $\times$ 3 sizes $\times$ 2 seeds.}
\label{tab:appendixb}
\small
\begin{tabular}{lrrrrrrr}
\toprule
Topology & Rooms & Exits & Variables & Corridor diam. & Bridges & Crit.\ bridges & MILP optimum (S1) \\
\midrule
Linear & 10 & 2 & 20 & 9 & 31 & 11 & 1.1940 \\
Linear & 20 & 4 & 80 & 19 & 63 & 23 & 0.3971 \\
Linear & 30 & 6 & 180 & 29 & 95 & 35 & 0.2440 \\
Tree & 10 & 2 & 20 & 7 & 35 & 11 & 1.7241 \\
Tree & 20 & 4 & 80 & 7 & 65 & 17 & 1.1002 \\
Tree & 30 & 6 & 180 & 11 & 107 & 30 & 0.7397 \\
Loop & 10 & 2 & 20 & 4 & 22 & 2 & 3.0133 \\
Loop & 20 & 4 & 80 & 8 & 44 & 4 & 0.7147 \\
Loop & 30 & 6 & 180 & 12 & 66 & 6 & 0.4056 \\
\bottomrule
\end{tabular}
\end{table}

\section{Reproducibility}
Source code, all 23 floorplan datasets (five real, 18 synthetic), generated route catalogs, validation tests, MILP/Neal/Hybrid benchmark outputs, and this manuscript's source are available at \url{https://github.com/RomeroNatalia/EvacCapstone} (branch \texttt{expansion/milp-optimality-gaps}), forked from and building on the original study's repository at \url{https://github.com/afishman2023/EvacCapstone}.
